...
require_container() {
    ...
    # RUNNING IS NOT READY. The docs' own remove section restarts Velociraptor,
    # and the next command ran against an engine that answers nothing: install
    # then said "no download URL known for Takajo-2.5.0" for a tool whose
    # artifact declares one. So when the container started moments ago, wait for
    # it. Nothing to pay in the normal case: the age check is one docker inspect.
    local age
    age="$(container_age_seconds)"
    [[ "$age" =~ ^[0-9]+$ ]] || return 0
    (( age < ${VELO_RECENT_START:-180} )) || return 0
    local i
    for (( i = 0; i <= ${VELO_READY_WAIT:-60}; i += 3 )); do
        engine_answering && return 0
        (( i == 0 )) && log "Velociraptor restarted ${age}s ago — waiting for its query engine…"
        sleep 3
    done
    err "Velociraptor is running but its query engine is still not answering."
    err "  Look: docker logs --tail 50 ${VELO_CONTAINER}"
    return 1
}
...
container_age_seconds() {
    ...
    VELO_STARTED="$started" python3 -c '
import os, datetime
try:
