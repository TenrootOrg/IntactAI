...
def _load_artifact_map(skills_dir: str) -> None:
    """Load skills_dir/artifact_map.yaml into _ARTIFACT_MAP, then overlay
    skills_dir/artifact_map_overrides.yaml on top. The base file is
    machine-generated by scripts/generate_skill_map.py; the overrides
    file is hand-tuned and is the right place to pin patterns the
    generator can't infer (e.g. cloud artifacts whose names don't
    appear in the Velociraptor catalog the generator scans).

    Silently leaves the map empty if both files are absent or
    unparseable — selector falls back to fuzzy match.
    """
    global _ARTIFACT_MAP
    base_path = os.path.join(skills_dir, "artifact_map.yaml")
    overrides_path = os.path.join(skills_dir, "artifact_map_overrides.yaml")
    ...
    if os.path.isfile(base_path):
        try:
            ...
            base_exact = data.get("exact") or {}
            base_patterns = data.get("patterns") or []
            if isinstance(base_exact, dict):
                exact.update(base_exact)
            else:
        ...
        logger.info("%s no artifact_map.yaml at %s — fuzzy match only", LOG_PREFIX_SKILLS, base_path)

    # Overlay overrides. Exact entries win over the generated map; pattern
    # entries from overrides are checked BEFORE generated patterns (so a
    # hand-tuned cloud pattern beats a fuzzy-generated endpoint one).
    overrides_exact = 0
    overrides_patterns = 0
    if os.path.isfile(overrides_path):
        try:
            with open(overrides_path, "r", encoding="utf-8") as f:
                ov = yaml.safe_load(f) or {}
            ov_exact = ov.get("exact") or {}
            ov_patterns = ov.get("patterns") or []
            if isinstance(ov_exact, dict):
                exact.update(ov_exact)  # overrides win
                overrides_exact = len(ov_exact)
            if isinstance(ov_patterns, list):
                patterns = list(ov_patterns) + list(patterns)
                overrides_patterns = len(ov_patterns)
            logger.info(
                "%s overlay artifact_map_overrides.yaml: %d exact, %d patterns",
                LOG_PREFIX_SKILLS, overrides_exact, overrides_patterns,
            )
        except (OSError, yaml.YAMLError) as e:
            logger.warning("%s failed to read artifact_map_overrides.yaml: %s", LOG_PREFIX_SKILLS, e)

    _ARTIFACT_MAP = {"exact": exact, "patterns": patterns}
    logger.info(
        "%s artifact_map total: %d exact entries, %d glob patterns",
...
def select_macro_skill(
    ...
    # Hard-pin: when the run's artifacts are predominantly Azure / Entra
    # cloud-IR (sign-ins, audits, UAL, INV state), prefer our own macro
    # over any endpoint-forensics one the fuzzy scorer might pick. The
    # endpoint macros pull the LLM toward Windows / disk / memory patterns
    # that don't fit a cloud chain.
