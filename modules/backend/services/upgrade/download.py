...
from services.upgrade.resolver import GITHUB_API


def _github_token():
    """Optional GitHub token from the backend environment (0705B backport:
    this release's resolver has no _github_token helper — it reads the env
    directly, same as resolver.get_github_rate_limit). The repo is public, so
    downloads also work unauthenticated; the token just raises rate limits."""
