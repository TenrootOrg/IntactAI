...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.store('settings', {
        ...
        // ---- Smart image cleanup state ----
        imagesLoading: false,
        imagesPruning: false,
        imagesData: null,            // {categories, totals_label, reclaimable_safe_label, pins}
        imagesSelected: new Set(),   // image_id (or ref fallback) of operator-chosen rows

        async loadUnusedImages() {
            this.imagesLoading = true;
            this.imagesSelected = new Set();
            try {
                const r = await fetch('/api/maintenance/unused-images');
                const d = await r.json();
                if (!r.ok) throw new Error(d.error || 'unknown error');
                this.imagesData = d;
                // Default-select the safe categories (module:older + dangling)
                for (const cat of ['module:older', 'dangling']) {
                    for (const img of (d.categories?.[cat] || [])) {
                        this.imagesSelected.add(img.image_id || img.ref);
                    }
                }
                // Force Alpine to re-render set-backed UI
                this.imagesSelected = new Set(this.imagesSelected);
            } catch (e) {
                this.showMessage('Could not load images: ' + e.message, 'error');
            } finally {
        ...
        toggleImage(img) {
            const key = img.image_id || img.ref;
            if (this.imagesSelected.has(key)) {
                this.imagesSelected.delete(key);
            } else {
                this.imagesSelected.add(key);
            }
            this.imagesSelected = new Set(this.imagesSelected);
        },

        selectedBytesLabel() {
            if (!this.imagesData) return '0 B';
            let bytes = 0;
            for (const entries of Object.values(this.imagesData.categories || {})) {
                for (const img of entries) {
                    if (this.imagesSelected.has(img.image_id || img.ref)) {
                        bytes += img.size_bytes || 0;
                    }
                }
            }
        ...
        async pruneSelectedImages() {
            if (this.imagesSelected.size === 0) return;
            const ids = Array.from(this.imagesSelected);
            const size = this.selectedBytesLabel();
            if (!confirm(`Remove ${ids.length} images, freeing ${size}? Current pinned versions are protected and will be refused if accidentally selected.`)) {
                return;
            }
            this.imagesPruning = true;
            try {
                const r = await fetch('/api/maintenance/unused-images/prune', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ image_ids: ids, dry_run: false }),
                });
                const d = await r.json();
                if (!r.ok) throw new Error(d.error || 'unknown error');
                const removedMsg = `Removed ${d.removed_count} images (${d.removed_label})`;
                const protMsg = d.skipped_protected?.length ? `, ${d.skipped_protected.length} protected refused` : '';
                const failMsg = d.failed_count ? `, ${d.failed_count} failed` : '';
                this.showMessage(removedMsg + protMsg + failMsg, d.failed_count ? 'warning' : 'success');
                // Refresh inventory to show updated state
                await this.loadUnusedImages();
            } catch (e) {
                this.showMessage('Prune failed: ' + e.message, 'error');
            } finally {
                this.imagesPruning = false;
            }
        },
