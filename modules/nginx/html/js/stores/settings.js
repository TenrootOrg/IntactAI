    ...
    Alpine.store('settings', {
        ...
        prepareManualScript() {
            ...
            return [
                ...
                '',
                '# 1. List this release\'s package assets. jq if you have it,',
                '#    python3 otherwise -- both ship almost everywhere.',
                'REL=$(curl -fsSL "${AUTH[@]}" \\',
                ...
                'fi',
                '[ -s assets.txt ] || { echo "no package assets on $TAG" >&2; exit 1; }',
                '',
        ...
        prepareManualScriptPs() {
            ...
            return [
                ...
                '$rel = Invoke-RestMethod -Headers $Auth `',
                '  -Uri "https://api.github.com/repos/$Repo/releases/tags/$Tag"',
                '$assets = $rel.assets | Where-Object { $_.name.StartsWith($Base) }',
                'if (-not $assets) { throw "no package assets on $Tag" }',
                '',
                '# 2. Download each. A private repo serves assets from the API url',
                '#    with an octet-stream Accept header, NOT browser_download_url.',
                'foreach ($a in $assets) {',
