    ...
    Alpine.store('settings', {
        ...
        prepareManualScript() {
            ...
            return [
                ...
                '',
                ': "${GITHUB_TOKEN:?Set GITHUB_TOKEN first - github.com/settings/tokens, classic token, repo scope}"',
                '',
                'export BASE="intact-upgrade-$TAG.tar.gz"',
                'AUTH=(-H "Authorization: Bearer $GITHUB_TOKEN"',
                '      -H "X-GitHub-Api-Version: 2022-11-28")',
                '',
                '# 1. List this release\'s package assets. jq if present, python3 otherwise.',
                'if ! REL=$(curl -fsSL "${AUTH[@]}" "$API/repos/$REPO/releases/tags/$TAG"); then',
                '  echo "Cannot read release $TAG. A 401 means the token is wrong or lacks repo" >&2',
                '  echo "scope; a 404 means the tag does not exist." >&2',
                '  exit 1',
                'fi',
                '',
                'if command -v jq >/dev/null; then',
                '  printf \'%s\' "$REL" | jq -r --arg b "$BASE" \\',
                '    \'.assets[] | select(.name|startswith($b)) | "\\(.name) \\(.url)"\' > assets.txt',
                ...
                'fi',
                '[ -s assets.txt ] || { echo "Release $TAG has no package assets (CI may still be building it)." >&2; exit 1; }',
                '',
        ...
        prepareManualScriptPs() {
            ...
            return [
                ...
                '  $Api  = \'https://api.github.com\'',
                '',
                '  # SINGLE quotes: in double quotes PowerShell expands $env:GITHUB_TOKEN',
                '  # inside the message itself, so an unset token printed "set  first".',
                '  if (-not $env:GITHUB_TOKEN) {',
                '    throw \'Set $env:GITHUB_TOKEN first - github.com/settings/tokens, classic token, repo scope.\'',
                '  }',
                '',
                ...
                '  } catch {',
                '    throw "Cannot read release $Tag - $($_.Exception.Message). A 401 means the token is wrong or lacks repo scope; a 404 means the tag does not exist."',
                '  }',
