...
# plan.sh OWNS this; core.sh only reads it (_u_unwind_current has to know
# whether the module that failed was being installed for the first time, since
# there is then no previous version to be "restored to").
#
# Declared here as well so core.sh survives being sourced on its own. On an
# UNDECLARED name, bash treats `[$module]` as an ARITHMETIC subscript rather
# than a string key -- so `${PLAN_ACTION[$module]:-}` evaluates `elk` as a
# variable name and dies under `set -u` with "elk: unbound variable". The `:-`
# never gets a chance: the failure happens while resolving the subscript, not
# the value. That is what broke test_upgrade_core.sh and test_upgrade_health.sh,
# both of which source core.sh without plan.sh.
#
# No `=()`: on an array that already has contents this is a no-op, so sourcing
# order stays irrelevant. plan.sh's own `declare -gA PLAN_ACTION=()` is what
# resets it per run.
declare -gA PLAN_ACTION 2>/dev/null || true
